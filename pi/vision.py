...
def verificacao_tripla_verde(contornos_reais, config):
    global contador_verde
    verde_valido_neste_frame = False
    
    
    for c in contornos_reais:
        # 1. Teste de Área Real do OpenCV
        area = float(cv2.contourArea(c))
        if area < config.area_minima_verde:
            continue
            
        
        x, y, w, h = cv2.boundingRect(c)
        relacao_aspecto = float(w) / h
        
               if not (0.7 <= relacao_aspecto <= 1.3):
            continue 
            

        verde_valido_neste_frame = True
        break

    
    if verde_valido_neste_frame:
        contador_verde += 1
    else:
        contador_verde = 0 
        
    
    if contador_verde >= FRAMES_PARA_CONFIRMAR:
        return True 
...
def _detectar_verde(quadro_bgr, configuracao):
    ...
    melhor = None
 verde_confirmado = verificacao_tripla_verde(contornos, configuracao)

    
    if not verde_confirmado:
        return {
